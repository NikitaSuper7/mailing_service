...
class Command(BaseCommand):
    def handle(self, *args, **kwargs):
        self.attempts_mailing()

    # @staticmethod
    def attempts_mailing(self):
        """Отправляем рассылку"""
        zone = timezone(settings.TIME_ZONE)
        current_datetime = datetime.now(zone)
        for mailing in Mailing.objects.filter(message_states="Created"):
            if mailing.sent_at and mailing.end_send_at and mailing.sent_at < current_datetime < mailing.end_send_at:
                ...
                try:
                    ...
                    attempt = Attemts.objects.create(mailing=mailing, server_response=server_response)
                    if server_response:
                        attempt.state = "Успешно"
